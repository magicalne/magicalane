...
fn decode_recv(
    ...
    let addr = match libc::c_int::from(name.ss_family) {
        libc::AF_INET => unsafe {
            // FIX (magicalane): std::net::SocketAddrV4 is NOT layout-compatible
            // with libc::sockaddr_in on modern rustc, so the original
            // `ptr::read::<SocketAddrV4>` mis-decoded the source address
            // (e.g. 10.89.0.6:40726 became 2.0.159.22:22794). Decode field by
            // field instead.
            let sin: libc::sockaddr_in = ptr::read(&name as *const _ as _);
            SocketAddr::new(
        ...
        libc::AF_INET6 => unsafe {
            // FIX (magicalane): same as above for IPv6.
            let sin6: libc::sockaddr_in6 = ptr::read(&name as *const _ as _);
            SocketAddr::new(
