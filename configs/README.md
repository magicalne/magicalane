...
| [`client-gateway.toml`](client-gateway.toml) | LAN bypass-router gateway (`gateway = true, bypass_router = true`: intercepts forwarded traffic, serves as default route + DNS incl. ICMP and DNS-over-TCP) — deployed by `scripts/gateway.sh`, see `docs/deploy-gateway.md` |
